...
export function Conversation({
        ...
        const loadConversation = async () => {
            try {
                ...
                if (data.conversation && data.conversation.messages) {
                    const messages = data.conversation.messages.map(
    ...
    const conversation = useConversation({
        ...
        onMessage: (message) => {
            console.log(
                "Message received (full object):",
                JSON.stringify(message, null, 2)
            );

            // ElevenLabs message structure: { source: "user" | "ai", message: string }
            const messageText = message.message || "";
            let messageRole: "user" | "assistant" | null = null;

            // Map ElevenLabs source to our role format
            ...
            if (messageRole && messageText.trim()) {
                console.log(
                    `Adding ${messageRole} message: "${messageText.substring(
                        0,
                        50
                ...
                setConversationHistory((prev) => {
                    const updated = [
                        ...prev,
                        { role: messageRole!, content: messageText.trim() },
                    ];
                    console.log(
                        `Updated history: ${updated.length} messages total`
                    );
                    return updated;
                });
            } else {
            ...
            <div className="flex flex-col items-center gap-2">
                ...
                {isUserLoaded && !user?.id && (
                    <p className="text-sm text-red-600">
                        ⚠️ Please sign in to save conversations
                    </p>
                )}
                {conversationHistory.length === 0 &&
                    conversation.status !== "connected" && (
